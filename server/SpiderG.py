...
def test_move():
    ...
    legs = {
        "Front Left": {"channel_offset": 0, "channels": {0: 0, 1: 0, 2: 0}},
        "Front Right": {"channel_offset": 3, "channels": {0: 0, 1: 0, 2: 0}},
        "Back Left": {"channel_offset": 6, "channels": {0: 0, 1: 0, 2: 0}},
        "Back Right": {"channel_offset": 9, "channels": {0: 0, 1: 0, 2: 0}},
    }

    def move(servo_channel, servo_rotation_angle, servo_deviation=0, delay_time=0.5):
        pwm.set_pwm(servo_channel, servo_deviation, servo_rotation_angle)
        time.sleep(delay_time)

    while True:
        for leg_name, data in legs.items():
            print("")
            print(leg_name)
            for channel, deviation in data["channels"].items():
                for rotation_angle in [servo_min, servo_max, servo_min]:
                    print(f"    {channel} -> {rotation_angle} | D: {deviation}")
                    move(
                        servo_channel=channel + data["channel_offset"],
                        servo_rotation_angle=rotation_angle,
                        servo_deviation=deviation,
                    )
...
def move_smooth_goal():
    ...
    print(f"")
    print(f"")
    print(f"move_smooth_goal")
    print(f"	now_command: `{now_command}`")
    print(f"	goal_command: `{goal_command}`")
    print(f"	gait_set: `{gait_set}`")
    print(f"	count_input: `{count_input}`")
    print(f"	total_count: `{total_count}`")
    print(f"	FLM_port: `{FLM_port}`")
    print(f"	FRM_port: `{FRM_port}`")
    print(f"	HLM_port: `{HLM_port}`")
    print(f"	HRM_port: `{HRM_port}`")
    ...
    for i in range(0, count_input):
        if goal_command != now_command:
            ...
            return 1
        now_dict["FLB"] = move_smooth_base(
            FLB_port, goal_dict["FLB"], old_dict["FLB"], i, count_input
        )
        now_dict["FLM"] = move_smooth_base(
            FLM_port, goal_dict["FLM"], old_dict["FLM"], i, count_input
        )
        now_dict["FLE"] = move_smooth_base(
            FLE_port, goal_dict["FLE"], old_dict["FLE"], i, count_input
        )

        now_dict["FRB"] = move_smooth_base(
            FRB_port, goal_dict["FRB"], old_dict["FRB"], i, count_input
        )
        now_dict["FRM"] = move_smooth_base(
            FRM_port, goal_dict["FRM"], old_dict["FRM"], i, count_input
        )
        now_dict["FRE"] = move_smooth_base(
            FRE_port, goal_dict["FRE"], old_dict["FRE"], i, count_input
        )

        now_dict["HLB"] = move_smooth_base(
            HLB_port, goal_dict["HLB"], old_dict["HLB"], i, count_input
        )
        now_dict["HLM"] = move_smooth_base(
            HLM_port, goal_dict["HLM"], old_dict["HLM"], i, count_input
        )
        now_dict["HLE"] = move_smooth_base(
            HLE_port, goal_dict["HLE"], old_dict["HLE"], i, count_input
        )

        now_dict["HRB"] = move_smooth_base(
            HRB_port, goal_dict["HRB"], old_dict["HRB"], i, count_input
        )
        now_dict["HRM"] = move_smooth_base(
            HRM_port, goal_dict["HRM"], old_dict["HRM"], i, count_input
        )
        now_dict["HRE"] = move_smooth_base(
            HRE_port, goal_dict["HRE"], old_dict["HRE"], i, count_input
        )
...
def goal_GenOut(position_input, left_direction, right_direction):
    def leg_FL(pos, direction_input):
        if pos == 1:
            ...
            goal_dict["FLM"] = int(
                FLM_init_pwm + (wiggle_v - FL_height) * FLM_direction
            )
            goal_dict["FLE"] = int(FLE_init_pwm + (wiggle_v + 0) * FLE_direction)
        elif pos == 2:
            goal_dict["FLB"] = int(
                FLB_init_pwm
                + (wiggle_middle + wiggle_h * direction_input) * FLB_direction
            )
            ...
            goal_dict["FLB"] = int(
                ...
                + (
                        wiggle_middle
                        + (wiggle_h * (6 - (pos - 2)) / 3 - wiggle_h) * direction_input
                )
                * FLB_direction
            )
            goal_dict["FLM"] = int(FLM_init_pwm - FL_height * FLM_direction)
    ...
    def leg_FR(pos, direction_input):
        if pos == 1:
            ...
            goal_dict["FRM"] = int(
                FRM_init_pwm + (wiggle_v - FR_height) * FRM_direction
            )
            goal_dict["FRE"] = int(FRE_init_pwm + (wiggle_v + 0) * FRE_direction)
        elif pos == 2:
            goal_dict["FRB"] = int(
                FRB_init_pwm
                + (wiggle_middle + wiggle_h * direction_input) * FRB_direction
            )
    ...
    def leg_HL(pos, direction_input):
        if pos == 1:
            ...
            goal_dict["HLM"] = int(
                HLM_init_pwm + (wiggle_v - HL_height) * HLM_direction
            )
            goal_dict["HLE"] = int(HLE_init_pwm + (wiggle_v + 0) * HLE_direction)
        elif pos == 2:
            goal_dict["HLB"] = int(
                HLB_init_pwm
                + (-wiggle_middle + wiggle_h * direction_input) * HLB_direction
            )
    ...
    def leg_HR(pos, direction_input):
        if pos == 1:
            ...
            goal_dict["HRM"] = int(
                HRM_init_pwm + (wiggle_v - HR_height) * HRM_direction
            )
            goal_dict["HRE"] = int(HRE_init_pwm + (wiggle_v + 0) * HRE_direction)
        elif pos == 2:
            goal_dict["HRB"] = int(
                HRB_init_pwm
                + (-wiggle_middle + wiggle_h * direction_input) * HRB_direction
            )
...
def status_GenOut(height_input, pitch_input, roll_input):
    ...
    def leg_FL_status():
        ...
        goal_dict["FLM"] = ctrl_range(
            int(FLM_init_pwm + (height_input + FL_input) * FLM_direction),
            max_dict["FLM"],
            min_dict["FLM"],
        )
    ...
    def leg_FR_status():
        ...
        goal_dict["FRM"] = ctrl_range(
            int(FRM_init_pwm + (height_input + FR_input) * FRM_direction),
            max_dict["FRM"],
            min_dict["FRM"],
        )
    ...
    def leg_HL_status():
        ...
        goal_dict["HLM"] = ctrl_range(
            int(HLM_init_pwm + (height_input + HL_input) * HLM_direction),
            max_dict["FRM"],
            min_dict["FRM"],
        )
    ...
    def leg_HR_status():
        ...
        goal_dict["HRM"] = ctrl_range(
            int(HRM_init_pwm + (height_input + HR_input) * HRM_direction),
            max_dict["FRM"],
            min_dict["FRM"],
        )
            ...
            if not steadyMode:
                ...
                while move_smooth_goal():
                    if goal_command == "stop":
                        ...
                        continue
                if (
                        goal_command == "StandUp"
                        or goal_command == "StayLow"
                        or goal_command == "Lean-L"
                        or goal_command == "Lean-R"
                ):
                    servoStop()
            else:
                steady()
